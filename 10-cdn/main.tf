resource "aws_cloudfront_distribution" "mydevops3" {
  enabled             = true
  ...
  records = [
    {
      ...
      type    = "A"
      alias   = {
        name    = aws_cloudfront_distribution.mydevops3.domain_name
        zone_id = aws_cloudfront_distribution.mydevops3.hosted_zone_id
      }
    }
  ]
}